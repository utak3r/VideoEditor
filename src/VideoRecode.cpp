...
void VideoRecode::recode()
{
    ...
    if (openMedia(decoder->filename, &decoder->avfc) == 0)
    {
        if (prepareDecoder(decoder) == 0)
        {
            ...
            // Preparing data for seeking and skipping
            bool video_mark_in_reached = false;
            bool video_mark_out_reached = false;
			bool audio_mark_out_reached = false;
			int64_t start_ms = theMarks.MillisecondsStart();
            AVRational video_time_base = decoder->avfc->streams[decoder->video_index]->time_base;
			AVRational audio_time_base = decoder->avfc->streams[decoder->audio_index]->time_base;
            int64_t video_start_pts = millisecondsToTimestamp(start_ms, video_time_base);
            int64_t audio_start_pts = millisecondsToTimestamp(start_ms, audio_time_base);
            ...
            while (av_read_frame(decoder->avfc, input_packet) >= 0)
            {
                if (decoder->avfc->streams[input_packet->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
                {
                    ...
                    {
                        if (remux(&input_packet, &encoder->avfc, decoder->video_avs->time_base, encoder->video_avs->time_base, video_start_pts, end_pts)) return;
                    }
                    frame_count++;
                }
                else if (decoder->avfc->streams[input_packet->stream_index]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO)
                {
                    ...
                    {
                        if (remux(&input_packet, &encoder->avfc, decoder->audio_avs->time_base, encoder->audio_avs->time_base, audio_start_pts, end_pts)) return;
                    }
                }
                else
                {
