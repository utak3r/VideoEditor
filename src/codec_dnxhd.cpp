...
{
	return ff->avcodec_find_encoder_by_name("dnxhd");
}
...
void CodecDnXHD::setPreset(const QString& preset, AVCodecContext* codecContext)
{
	if (codecContext && codecContext->priv_data)
	{
		...
		{
			ff->av_opt_set(codecContext->priv_data, "profile", "dnxhr_90", 0);
			...
			codecContext->time_base = ff->av_inv_q(codecContext->framerate);
		}
		else if (preset == "1280x720p 220Mbps yuv422p")
		{
			ff->av_opt_set(codecContext->priv_data, "profile", "dnxhr_hq", 0);
		...
		{
			ff->av_opt_set(codecContext->priv_data, "profile", "dnxhr_185", 0);
			//av_opt_set(codecContext->priv_data, "pix_fmt", "yuv422p10", 0);
		...
		{
			ff->av_opt_set(codecContext->priv_data, "profile", "dnxhr_444", 0);
			//av_opt_set(codecContext->priv_data, "pix_fmt", "yuv422p10", 0);
